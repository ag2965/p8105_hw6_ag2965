...
homicides_clean=homicides_df%>%
  janitor::clean_names()%>%
    ...
    filter(city_state !='Dallas, TX' |city_state != 'Phoenix, AZ'|city_state != 'Kansas City, MO'| city_state != 'Tulsa, OK')%>%
  mutate(
...
```{r}
balt_fit = glm(solved~victim_age + victim_sex + victim_race, data=baltimore_homicides)
...
```{r}
homicides_clean %>% 
  lm(solved~victim_age + victim_sex + victim_race, data = .) %>% 
  broom::tidy() %>% 
...
homicide_fit =
  ...
  mutate(
    models = map(data, ~lm(solved~victim_age + victim_sex + victim_race, data = .x)),
